...
La pako `esperantilo` estas nuntempe skeleto, do ĉi tiu paĝo estas **dezajna celo**:
ĝi montras kiel aspektas pura NLP-interfaco —modelita laŭ [spaCy](https://spacy.io/api)—
por ke la propra interfaco `Doc` / `Token` de la biblioteko povu esti konstruita
imitante ĝin.
...
kion li bezonas por voki `parse` ĝuste, sen legi ĝian korpon.
