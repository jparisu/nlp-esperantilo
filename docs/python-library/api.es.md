...
El paquete `esperantilo` es actualmente un esqueleto, así que esta página es un
**objetivo de diseño**: muestra cómo es una interfaz de PLN limpia —modelada sobre
[spaCy](https://spacy.io/api)— para que la propia interfaz `Doc` / `Token` de la
biblioteca pueda construirse imitándola.
