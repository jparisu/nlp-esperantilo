...
│       └── __init__.py
└── tests/
    ├── test_package.py   # pruebas de humo del paquete
...
Un archivo `__init__.py` es lo que convierte un directorio en un **paquete**: sin
él, Python no trata la carpeta como importable. Se ejecuta cuando el paquete se
importa por primera vez, y define la **superficie pública** del paquete. El de
este proyecto es deliberadamente mínimo:
...
La versión de la biblioteca vive en **un solo sitio**, `version` en
`pyproject.toml`, y se refleja en `__version__` en `__init__.py` para que sea
legible en tiempo de ejecución:
