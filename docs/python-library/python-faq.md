...
??? question "What is the difference between a module, a package and a library?"
    A **module** is a single `.py` file; a **package** is a folder of modules with
    an `__init__.py`; a **library** is a package meant to be reused by other code.
...
??? question "What does `__init__.py` do?"
    It marks a directory as a **package** and defines what the package exposes
    when imported. Here it declares the version and, as the library grows, is
    where public classes are re-exported. See
