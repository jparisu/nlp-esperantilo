...
??? question "¿Cuál es la diferencia entre un módulo, un paquete y una biblioteca?"
    ...
    una **biblioteca** es un paquete pensado para ser reutilizado por otro código.
...
??? question "¿Qué hace `__init__.py`?"
    ...
    *paquete de espacio de nombres*, pero una biblioteca debe ser explícita.) Aquí
    declara la versión y, a medida que la biblioteca crezca, es donde se reexportan
