...
??? question "Kio estas la diferenco inter modulo, pako kaj biblioteko?"
    **Modulo** estas unuopa dosiero `.py`; **pako** estas dosierujo de moduloj importata
    kiel unu unuo, kutime markita per `__init__.py`; **biblioteko** estas pako celita por
    esti reuzata de alia kodo.
...
??? question "Kion faras `__init__.py`?"
    Ĝi markas dosierujon kiel **regulan pakon** kaj difinas kion la pako eksponas ĉe la
    importo. (De Python 3.3, dosierujo sen ĝi restas importebla, kiel *nomspaca pako*, sed
    biblioteko estu eksplicita.) Ĉi tie ĝi deklaras la version kaj, dum la biblioteko
    kreskas, estas kie reeksportiĝas la publikaj klasoj. Vidu
    [Organizado § __init__.py](organization.md#__init__py).

??? question "Kiel mi instalas la bibliotekon en kajero de Google Colab?"
    ...
    Vidu [Instalado kaj uzo](installation-and-usage.md#uzi-gin-en-kajero).
...
??? question "Mi instalis novan version en kajero sed nenio ŝanĝiĝis. Kial?"
    ...
    ŝargita. Vidu [Instalado kaj uzo](installation-and-usage.md#uzi-gin-en-kajero).
