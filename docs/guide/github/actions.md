...
builds this site and publishes it. It runs **on push to `main`** (and on demand),
because the public site should reflect merged work, not work in progress:

```yaml
on:
  push:
    branches: [main]
  workflow_dispatch:          # re-deploy by hand from the Actions tab
