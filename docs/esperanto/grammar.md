...
This is the core reference page of the Esperanto section. It collects **all the
regular rules** a rule-based pipeline needs — for the tokenizer, the lemmatizer,
the POS tagger and stop-word handling. Because Esperanto has no exceptions, each
rule below can be turned almost directly into code.
...
one letter. Six letters carry diacritics found in no other language:
...
!!! warning "Encoding matters for tokenization"
    ...
    [`resources/README.md`](vocabulary.md#format)).
...
The `-u`, `-o` and `-a` correlatives can themselves take `-j` and `-n` (`tiujn`,
`kiuj`), like the nouns and adjectives they resemble. Most correlatives are
stop-words.
...
as the head: `vapor-ŝipo` (steamship), `dorm-oĉambro` (bedroom, with a connecting
