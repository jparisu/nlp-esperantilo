...
the concrete data the library loads: the stop-words, the full affix inventory,
and a starter set of common roots. The lists here are meant to be copied straight
...
!!! abstract "The full list already exists as data"
    ...
    **[Stop-words →](word-lists/ignorindaj-vortoj.md)** — 250 entries, each with
    ...
    from `resources/ignorindaj-vortoj.json`, the very file the library reads, so
    the documentation and the code can never disagree.
...
so that a single source of truth feeds both the code (at runtime) and the
documentation (at build time, via [`hooks/word_lists.py`](../python-library/organization.md)).
...
Only `id`, `titolo` and `vortoj` are mandatory. Files are UTF-8 and keep the real
diacritics (`ĉ ĝ ĥ ĵ ŝ ŭ`), never the x-system. Every file is validated by
[`tests/test_resources.py`](../python-library/testing.md#the-tests-structure), so
a malformed edit fails in CI rather than in a documentation build. The full schema
is documented in `resources/README.md`.
