...
datos concretos que carga la biblioteca: las palabras vacías, el inventario
...
!!! abstract "La lista completa ya existe como datos"
    ...
    **[Palabras vacías →](word-lists/ignorindaj-vortoj.md)** — 250 entradas, cada
    ...
    genera a partir de `resources/ignorindaj-vortoj.json`, el mismo archivo que lee
    la biblioteca, de modo que la documentación y el código nunca pueden discrepar.
...
`resources/`**, para que una única fuente de verdad alimente tanto el código (en
tiempo de ejecución) como la documentación (en tiempo de construcción, mediante
[`hooks/word_lists.py`](../python-library/organization.md)). Los nombres de los
campos están en Esperanto, coincidiendo con los nombres de archivo; los campos
traducibles son objetos indexados por locale:
...
Solo `id`, `titolo` y `vortoj` son obligatorios. Los archivos son UTF-8 y conservan
los diacríticos reales (`ĉ ĝ ĥ ĵ ŝ ŭ`), nunca el sistema x. Cada archivo lo valida
[`tests/test_resources.py`](../python-library/testing.md#la-estructura-tests), de
modo que una edición mal formada falla en CI en lugar de en una construcción de la
documentación. El esquema completo está documentado en `resources/README.md`.
