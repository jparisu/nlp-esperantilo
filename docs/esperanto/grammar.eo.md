...
Ĉi tiu estas la centra referenca paĝo de la Esperanto-sekcio. Ĝi kolektas **ĉiujn
regulajn regulojn**, kiujn bezonas regul-bazita ĉenstrukturo — por la tokenizilo, la
lematizilo, la vortklasa markilo kaj la traktado de ignorindaj vortoj. Ĉar Esperanto ne
havas esceptojn, ĉiun sub-regulon eblas transformi preskaŭ rekte en kodon.
...
<!-- TODO: Ĉi tiu paĝo estis maŝine tradukita; petu kontrolon de denaska aŭ flua
Esperantisto pri terminologio (ekz. "vortklaso", "fleksio", "ĉenstrukturo"). -->
...
Ses literoj portas diakritajn signojn troveblajn en neniu alia lingvo:

| Litero | Nomo | Sono |
| --- | --- | --- |
| `ĉ` | ĉo | kiel *ĉ* en "ĉevalo" |
| `ĝ` | ĝo | kiel *ĝ* en "ĝardeno" (esprimita) |
...
!!! warning "La kodoprezento gravas por la tokenizado"
    ...
    (vidu [`resources/README.md`](vocabulary.md#formato)).
...
| Finaĵo | Vortklaso | Ekzemplo | Radiko | Signifo |
| --- | --- | --- | --- | --- |
| `-o` | substantivo | `libro` | `libr-` | libro |
| `-a` | adjektivo | `bona` | `bon-` | bona |
| `-e` | adverbo (derivita) | `rapide` | `rapid-` | rapide |
| `-i` | verbo (infinitivo) | `kanti` | `kant-` | kanti |
...
radikon, kaj poste realigu la vortklasan vokalon por akiri la vortaran formon.
...
| `ni` | ni | `nia` |
| `ili` | ili | `ilia` |
| `oni` | oni (senpersona) | `onia` |
| `si` | refleksiva (sin) | `sia` |
...
La korelativoj en `-u`, `-o` kaj `-a` povas mem preni `-j` kaj `-n` (`tiujn`, `kiuj`),
kiel la substantivoj kaj adjektivoj, al kiuj ili similas. La plimulto de la korelativoj
estas ignorindaj vortoj.
...
kiel kapo: `vapor-ŝipo` (vaporŝipo), `dorm-oĉambro` (dormoĉambro, kun ligilo `-o-` por
...
| `juna` | `jun-` + `-a` | adjektivo — "juna" |
| `instruistino` | `instru-` + `-ist-` + `-in-` + `-o` | substantivo — "instruistino" |
| `legas` | `leg-` + `-as` | verbo, prezenco — "legas" (lemo `legi`) |
| `interesan` | `interes-` + `-a` + `-n` | adjektivo, akuzativo — "interesa" |
| `libron` | `libr-` + `-o` + `-n` | substantivo, akuzativo — "libro" (lemo `libro`) |
