...
konkretajn datumojn, kiujn ŝargas la biblioteko: la ignorindajn vortojn, la plenan
...
oftaj adverboj. NLP-ĉenstrukturo kutime filtras ilin antaŭ la analizo, por ke `la`,
...
!!! abstract "La plena listo jam ekzistas kiel datumoj"
    Ĉi tiu projekto distribuas la plenan liston kiel maŝinlegeblan dosieron, redonatan
    ĉi tie aŭtomate:
    ...
    **[Ignorindaj vortoj →](word-lists/ignorindaj-vortoj.md)** — 250 eroj, ĉiu kun sia
    ...
    `resources/ignorindaj-vortoj.json`, la sama dosiero kiun legas la biblioteko, por
    ke la dokumentaro kaj la kodo neniam povu malkonsenti.
...
por ke unu sola fonto de vero nutru kaj la kodon (rultempe) kaj la dokumentaron
(konstrutempe, per [`hooks/word_lists.py`](../python-library/organization.md)). La nomoj
de la kampoj estas en Esperanto, kongruaj kun la dosiernomoj; la tradukeblaj kampoj
estas objektoj indeksitaj laŭ lingvokodo:
...
Nur `id`, `titolo` kaj `vortoj` estas devigaj. La dosieroj estas UTF-8 kaj konservas la
realajn diakritajn signojn (`ĉ ĝ ĥ ĵ ŝ ŭ`), neniam la x-sistemon. Ĉiun dosieron
validigas [`tests/test_resources.py`](../python-library/testing.md#la-strukturo-tests),
por ke misformita redakto malsukcesu en CI anstataŭ en konstruo de la dokumentaro. La
plena skemo estas dokumentita en `resources/README.md`.
