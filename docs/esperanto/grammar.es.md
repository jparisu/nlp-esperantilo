...
Esta es la página de referencia central de la sección de Esperanto. Reúne **todas
las reglas regulares** que necesita una tubería (*pipeline*) basada en reglas —
para el tokenizador, el lematizador, el etiquetador morfosintáctico y el manejo de
palabras vacías. Como el Esperanto no tiene excepciones, cada regla de abajo puede
convertirse casi directamente en código.
...
exactamente una letra. Seis letras llevan diacríticos que no se encuentran en
ninguna otra lengua:
...
!!! warning "La codificación importa para la tokenización"
    ...
    y nunca el sistema x (véase [`resources/README.md`](vocabulary.md#formato)).
...
Los correlativos en `-u`, `-o` y `-a` pueden tomar a su vez `-j` y `-n` (`tiujn`,
`kiuj`), como los sustantivos y adjetivos a los que se parecen. La mayoría de los
correlativos son palabras vacías.
...
`dorm-oĉambro` (dormitorio, con una `-o-` de enlace para facilitar la
