...
??? question "Kiel funkcias la lematizado en Esperanto?"
    ...
    poste realigu la vortklasan vokalon. `malsanulejojn` → forigu `-n`, `-j`, `-o`,
...
??? question "De kie la biblioteko prenas siajn vortlistojn?"
    El JSON-dosieroj sub `resources/`, kiuj estas la sola fonto de vero: la biblioteko
    legas ilin rultempe kaj la dokumentaro redonas ilin konstrutempe, do ili neniam
    povas malkonsenti. Vidu [Vortprovizo § Formato](vocabulary.md#formato).
